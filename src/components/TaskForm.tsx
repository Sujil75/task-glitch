...
export default function TaskForm({
  ...
  const duplicateTitle = useMemo(() => {
    ...
    const compareTitles = initial
      ? existingTitles.filter(
          t => t.trim().toLowerCase() !== initial.title.trim().toLowerCase()
        )
      : existingTitles;

    return compareTitles.some(
      t => t.trim().toLowerCase() === current
    );
      ...
      <DialogContent>
        <Stack spacing={2} mt={1}>
          <TextField
            ...
            error={Boolean(title) && duplicateTitle}
            helperText={
              duplicateTitle ? 'Duplicate title not allowed' : ' '
            }
            required
            autoFocus
          />
