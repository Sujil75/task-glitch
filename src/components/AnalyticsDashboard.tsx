...
export default function AnalyticsDashboard({ tasks }: Props) {
  ...
  const safeTasks: Task[] = tasks.map(t => ({
    ...
    completedAt: t.completedAt || undefined, // some functions handle undefined
  }));

  const funnel = computeFunnel(safeTasks);
  const weekly = computeThroughputByWeek(safeTasks);
  const weightedPipeline = computeWeightedPipeline(safeTasks);
  const forecast = computeForecast(
    weekly.map(w => ({ week: w.week, revenue: w.revenue })),
    4
  );
  const velocity = computeVelocityByPriority(safeTasks);
      ...
      <CardContent>
        ...
        <Stack spacing={3}>
          ...
          <Box>
            ...
            <PieChart
              height={240}
              series={[{ data: [{ id: 'pipeline', label: 'Weighted Revenue', value: weightedPipeline }] }]}
            />
          </Box>
